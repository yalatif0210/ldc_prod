...
echo "0 3 1 * * cd $(pwd) && CONTAINER=\$(docker ps --format '{{.Names}}' --filter 'publish=80' | head -1) && [ -n \"\$CONTAINER\" ] && docker stop \"\$CONTAINER\"; docker run --rm -v certbot_certs:/etc/letsencrypt -p 80:80 certbot/certbot renew --standalone --quiet; [ -n \"\$CONTAINER\" ] && docker start \"\$CONTAINER\"; docker exec frontend nginx -s reload"
