#!/bin/bash
# ─────────────────────────────────────────────────────────────
# deploy-prod.sh — Rebuild et redémarre les services Docker (PRODUCTION / VPS)
# Sur VPS, ce script doit être lancé depuis le dossier parent de docker-compose.yml (ex: /home/ldc/production),
# après git pull:
#    chmod +x deploy-prod.sh          # une seule fois
#   ./deploy-prod.sh backend-clean   # premier déploiement avec la nouvelle dépendance Jackson
#   ./deploy-prod.sh backend         # déploiements suivants (cache Docker)
#
# Usage :
#   ./deploy-prod.sh              → rebuild tous les services
#   ./deploy-prod.sh backend      → rebuild ldc_backend (avec cache Docker)
#   ./deploy-prod.sh backend-clean → rebuild ldc_backend SANS cache (après changement pom.xml)
#   ./deploy-prod.sh frontend     → rebuild uniquement ldc_frontend
#   ./deploy-prod.sh watch        → mode watch (rebuild auto sur changements)
#   ./deploy-prod.sh logs         → affiche les logs backend en direct
# ─────────────────────────────────────────────────────────────
